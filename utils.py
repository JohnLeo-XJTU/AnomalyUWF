...
class CoOpContext(nn.Module):
    def __init__(self, clip_model, n_ctx=4, embedding_dim=512, num_classes=2, device='cuda'):
        super(CoOpContext, self).__init__()
        ...
        ctx_init = torch.empty(self.num_classes, self.n_ctx, self.embedding_dim)
        nn.init.normal_(ctx_init, std=0.02)
        self.ctx = nn.Parameter(ctx_init.to(device))  # [num_classes, n_ctx, embedding_dim]

        # Placeholder for class tokens
        self.class_tokens = None

    def set_class_names(self, class_names):
        # Tokenize class names (e.g., 'normal', 'abnormal')
        self.class_tokens = tokenize(class_names).to(self.device)  # [num_classes, seq_len]

    def forward(self, obj):
        if self.class_tokens is None:
            raise ValueError("Class names must be set using set_class_names before encoding.")

        batch_size = self.ctx.shape[0]
        prompts = []
        for i in range(batch_size):
            # Combine learnable context with class token
            ctx = self.ctx[i]  # [n_ctx, embedding_dim]
            class_token = self.clip_model.token_embedding(self.class_tokens[i]).unsqueeze(
                0)  # [1, seq_len, embedding_dim]
            class_token = class_token[:, -1]  # [1, embedding_dim]
            prompt = torch.cat([ctx, class_token], dim=0)  # [n_ctx + 1, embedding_dim]
            prompts.append(prompt)

        prompts = torch.stack(prompts, dim=0)  # [num_classes, n_ctx + 1, embedding_dim]
        # Pass through CLIP's text transformer
        text_features = self.clip_model.transformer(prompts.permute(1, 0, 2))[0][-1]  # [num_classes, embedding_dim]
        # Project to the final embedding space
        text_features = torch.matmul(text_features, self.clip_model.text_projection)  # [num_classes, embedding_dim]
        text_features = text_features / text_features.norm(dim=-1, keepdim=True)
        return text_features.t()  # [embedding_dim, num_classes]


def encode_text_with_coop(clip_model, obj, device, n_ctx=4):
    # Define class names for normal and abnormal
    class_names = [f"{obj} normal", f"{obj} abnormal"]

    # Initialize CoOp context model
    coop_model = CoOpContext(clip_model, n_ctx=n_ctx, embedding_dim=clip_model.text_projection.shape[1], device=device)
    coop_model.set_class_names(class_names)

    # Compute text features
    text_features = coop_model(obj)
    return coop_model, text_features
...
def encode_text_with_prompt_ensemble(model, obj, device):
    ...
    prompt_normal = ['{}', 'flawless {}', 'perfect {}', 'unblemished {}', '{} without flaw', '{} without defect',
                     '{} without damage']
    prompt_abnormal = ['damaged {}', 'broken {}', '{} with flaw', '{} with defect', '{} with damage', ' diseased {}',
                       'abnormal {}']
    prompt_state = [prompt_normal, prompt_abnormal]
    prompt_templates = ['a bad photo of a {}.', 'a low resolution photo of the {}.', 'a bad photo of the {}.',
                        'a cropped photo of the {}.', 'a bright photo of a {}.', 'a dark photo of the {}.',
...
def augment(fewshot_img, fewshot_mask=None):
    ...
    if fewshot_mask is not None:
        ...
        # rotate img
        for angle in [-np.pi / 8, -np.pi / 16, np.pi / 16, -np.pi / 32, np.pi / 32,np.pi / 8]:
            rotate_img = rot_img(fewshot_img, angle)
            augment_fewshot_img = torch.cat([augment_fewshot_img, rotate_img], dim=0)
